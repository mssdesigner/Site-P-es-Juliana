<?php
    ...
    /**
     * Customizer panel representing Evolve panel for Redux
     *
     * @package    WordPress
     ...
     */
    class Evolve_Redux_Customizer_Panel extends WP_Customize_Panel {
...
if(isset($this->section['icon'])){
        echo '<i class="'. $this->section['icon'] .'"></i>';
                    ...
                    ?>
                    <span class="screen-reader-text"><?php esc_html_e( 'Press return or enter to open this panel', 'evolve' ); ?></span>
                </h3>
        ...
        protected function content_template() {
            ...
            <li class="panel-meta customize-info redux-panel accordion-section <# if ( ! data.description ) { #> cannot-expand<# } #>">
                <button class="customize-panel-back" tabindex="-1">
                    <span class="screen-reader-text"><?php esc_attr_e( 'Back', 'evolve' ); ?></span></button>
                <div class="accordion-section-title">
				<span class="preview-notice"><?php
                        /* translators: %s is the site/panel title in the Customizer */
                        echo sprintf( __( 'You are customizing %s', 'evolve' ), '<strong class="panel-title">{{ data.title }}</strong>' );
                    ?></span>
                    <# if ( data.description ) { #>
                        <button class="customize-help-toggle dashicons dashicons-editor-help" tabindex="0" aria-expanded="false">
                            <span class="screen-reader-text"><?php esc_attr_e( 'Help', 'evolve' ); ?></span></button>
                        <# } #>
                </div>
                <# if ( data.description ) { #>
